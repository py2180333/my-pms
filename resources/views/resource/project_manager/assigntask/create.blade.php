...
@section('content')
    <div class="page-wrapper create_p" style="min-height: 262px;">
        ...
        <div class="content container-fluid">
            ...
            <div class="row mt-4">
                <div class="col-md-12">
                    <form action="{{ route('resource.assigntask.store') }}" class="needs-validation card" method="POST" enctype="multipart/form-data">
                        @csrf
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        projectSelect.addEventListener('change', function () {
            ...
            if (projectId) {
                // Fetch Milestones
                fetch(`/resource/get-milestones/${projectId}`)
                ...
                // // Fetch Resources
                // fetch(`/resource/get-resources/${projectId}`)
                //     .then(response => response.json())
                //     .then(data => {
                ...
                //         // not show project manager in project manager
        ...
        milestonesDropdown.addEventListener('change', function () {
            ...
            if (milestoneId) {
                fetch(`/resource/get-tasks/${milestoneId}`)
                    .then(response => response.json())
                    .then(data => {
        ...
        resourcesDropdown.addEventListener('change', function () {
            ...
            if (consultantId) {
                fetch(`/resource/get-resource-details/${consultantId}/${role}`) // pr add role 25-9-25
                    .then(response => response.json())
                    .then(data => {
    ...
    tasksDropdown.addEventListener('change', function () {
        ...
        if (params) {
            // Fetch Resources
            fetch(`/resource/get-resources?${params.toString()}`)
                .then(response => response.json())
                .then(data => {
                    let options = '<option value="">Select Resource</option>';
        ...
        if (taskId) {
            fetch(`/resource/get-assigned-tasks/${taskId}`)
                .then(response => response.json())
                .then(data => {
                    ...
                    data.forEach((task, index) => {
                        const row = `<tr>
                            ...
                            <td class="text-center">
                                <form action="/resource/delete-assigned-task/${task.id}" method="POST" onsubmit="return confirm('Are you sure you want to Delete This?');">
