...
@section('content')
    ...
    <div class="page-wrapper create_p">
        ...
        <div class="content container-fluid">
            ...
            <div class="row mt-4">
                <div class="col-md-12">
                    <form method="POST" class="needs-validation card" action="{{route('admin.projects.assignteam.store')}}" >
                        @csrf
                        <div class="form-group row">
                            ...
                            <div class="col-md-4">
                                ...
                                <select class="form-control form-select js-states single" id="assigned-toresource" name="team_id" required>
                                    <option value="" disabled selected>Select Resource</option>
                                    @foreach ($resources as $resource)
                                        <option data-name="{{ $resource->first_name }} {{$resource->last_name}}" 
                                                data-email="{{ $resource->email }}" data-designation="{{ $resource->designation }}"
                                                data-payment="{{ $resource->rate }} {{$resource->payment_type}}"
                                                value="{{ $resource->id }}">{{ $resource->username }} ({{ $resource->email }})
                                        </option>
                                    @endforeach
                                </select>
                            </div>
    ...
    {{-- <script>
        ...
        document.getElementById('projectSelectresource').addEventListener('change', function() {
         const projectId = this.value;
            ...
            // Send AJAX request to get consultants not assigned to the selected project
            fetch(`/admin/projects/getAvailableConsultants?project_id=${projectId}`)
            ...
            // Fetch assigned consultants and display in the "Assigned Team" section
            fetch(`/admin/projects/getAssignedConsultants?project_id=${projectId}`)
                .then(response => response.json())
                .then(data => {
        ...
        document.getElementById('assigned-toresource').addEventListener('change', function() {
            ...
            // Retrieve the data-name attribute value
                    ...
                    if (data.length > 0) {
                        data.forEach((item, index) => {
                            const row = `<tr>
                                ...
                                <td class="text-center">${item.consultant.rate} ${item.consultant.payment_type}</td>
                                <td class="text-center">
                                    <form action="/admin/assignteam/${item.id}/softdelete" method="POST" onsubmit="return confirm('Are you sure you want to permanently delete This?');">
