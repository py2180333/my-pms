$(document).ready(function () {
    ...
    function setDataFromUrl() {
        ...
        const company = urlParams.get('company_id'); // Retrieve company from the URL
        const status = urlParams.get('status'); // Retrieve status from the URL
        const invoiceType = urlParams.get('invoice_type'); // Retrieve invoiceType from the URL
        ...
        if (invoiceType) $('#invoice-type-filter').val(invoiceType);
    ...
    function fetchInvoices() {
        ...
        const invoiceType = $('#invoice-type-filter').val() || $('#invoice-type-filter').val('all').val(); // Get selected invoice type
        ...
        $.ajax({
            ...
            success: function (response) {
                    ...
                    rows += `
                        <tr>
                            ...
                            <td class="text-center d-flex">
                                <a href="#" data-id="${invoice.id}" class="ms-2 p-2 fs-6  edit-action my_icons invoice-edit "><i class="fa-solid fa-pen-to-square text-dark"></i></a>
                                <a href="#" data-bs-toggle="modal" data-id="${invoice.id}" class="invoice-view my_icons ms-2 p-2 fs-6 view-action" data-bs-target="#invoice-view-user"   
                                            class=" p-2 fs-6 my_icons"><i data-bs-placement="top" title="View" class="fa-solid fa-eye view text-success"
                                                ></i></a>
                                <form action="/admin/invoice/delete/${invoice.id}" method="POST" 
                                    onsubmit="return confirm('Are you sure you want to detele this ${invoice.invoice_number} Invoice ?');">
                ...
                if (invoiceType && invoiceType !== 'all') query.set('invoice_type', invoiceType);
                if (startDate) query.set('startDate', startDate);
                if (endDate) query.set('endDate', endDate);

                // Reset to the base URL when no company filter is selected
                const newUrl = `${window.location.origin}/admin/invoice/index${query.toString() ? `?${query.toString()}` : ''}`;
    ...
    setDataFromUrl();
    // Initial fetch - is perform in 'invoice list'
    fetchInvoices();
});
...
$(document).on('click','.invoice-edit',function(){
    var invoiceId = $(this).data('id');
        ...
        $.ajax({
            url: '/admin/invoice/edit/display/' + invoiceId, // Fetch invoice data
            method: 'GET',
            success: function (invoice) {
        //         // Fill the form fields with existing customer data
        //         if(response.projectCustomer == response.id){
        //             //code for copaney drop down
        //         }
                // console.log(invoice); 
                // $('#edit-form-invoice').html(invoice);
                $('#edit-form-invoice .modal-body').html(invoice.html);
                $('#edit-form-invoice').modal('show');
                $('#edit-form-invoice select[name="status"]').val(invoice.invoice.status);
                $('#edit-form-invoice form').attr('action', '/admin/invoice/edit/' + invoice.invoice.id);
            },
            error: function () {
