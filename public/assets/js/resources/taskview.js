$(document).ready(function () {
    ...
    function fetchMilestone() {
        ...
        if(projectId == 'all'){
            ...
            return $.ajax({
                url: `/resource/tasks/${projectId}/milestones`,
                method: "GET",
                success: function (response) {
                    if(response.length === 0){
                        $('#no-milestone').toggle(true);
                        $('#milestone-filter-task').toggle(false);
                        $('#no-milestone').html('<a href="/resource/projects/milestonecreate" style="color: blue; text-decoration: underline;">Create a new milestone</a>');
                    } else {
                        $('#milestone-filter-task').toggle(true);
                        $('#no-milestone').toggle(false);
    ...
    function fetchTasks() {
        ...
        $.ajax({
            ...
            success: function (response) {
                    ...
                    rows += `
                        <tr>
                            ...
                            <td class="text-center d-flex">
                                ...
                                <form action="/resource/tasks/${task.id}/force-delete" method="POST" onsubmit="return confirm('Are you sure you want to delete Task?');">
                ...
                const newUrl = `${window.location.origin}/resource/tasks/index${query.toString() ? `?${query.toString()}` : ''}`;
