...
$(document).ready(function () {
    ...
    function fetchInvoices() {
        ...
        $.ajax({
            //url: "{{ route('resource.invoice.filter') }}",
            url: '/resource/invoice/filter',
            method: "GET",
            data: { 
                 status: status,
            ...
            success: function (response) {
                        ...
                        <tr>
                            ...
                            <td class="text-center d-flex">
                                <a href="#" data-id="${invoice.id}" 
                                    class="ms-2 p-2 fs-6  edit-action my_icons invoice-edit ">
                ...
                const newUrl = `${window.location.origin}/resource/invoice/index${query.toString() ? `?${query.toString()}` : ''}`;
...
$(document).on('click','.invoice-view',function(){
    ...
    $.ajax({
        url: '/resource/invoiceview/' + InvoiceId,
        type: 'GET',
...
$(document).on('click','.invoice-edit',function(){
    var invoiceId = $(this).data('id');
        ...
        $.ajax({
            url: '/resource/invoice/edit/display/' + invoiceId, // Fetch invoice data
            method: 'GET',
        ...
        //         }
                ...
                $('#edit-form-invoice form').attr('action', '/resource/invoice/edit/' + invoice.invoice.id);
            },
            error: function () {
