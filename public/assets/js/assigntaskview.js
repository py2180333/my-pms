$(document).ready(function () {
    ...
    function fetchMilestone() {
        ...
        if(projectId == 'all'){
            ...
            return $.ajax({
                url: `/admin/get-milestones/${projectId}`,
                method: "GET",
    ...
    function fetchTask() {
        ...
        if(milestoneId == 'all'){
            ...
            return $.ajax({
                url: `/admin/get-tasks/${milestoneId}`,
                method: "GET",
                success: function (response) {
    ...
    function fetchResources(){
        ...
        if(projectId == 'all'){
            ...
            return $.ajax({
                url: `/admin/assigntask/filter/resources`,
                method: "GET",
                success: function (response) {
                    ...
                    response.projectManager.forEach(function (pm) {
                        options += `<option value="${pm.id}">
                            ${pm.first_name} 
                            ${pm.last_name} 
                            (${pm.email}) 
                            ${pm.username}
            ...
            return $.ajax({
                url: `/admin/get-resources/filter/${projectId}`,
                method: "GET",
                success: function (response) {
                    let options = '<option value="all" selected>All Resources</option>';
                    ...
                    response.resourcesPM.forEach(function (item) {
                        options += `<option value="${item.project.project_manager_id}">
                            ${item.project.manager.first_name} 
                            ${item.project.manager.last_name} 
                            (${item.project.manager.email}) 
                            ${item.project.manager.username} 
                            (PM)
                        </option>`;
                    }); 
                    $('#resource-filter-at-task').html(options);
    ...
    function fetchAssignTasks() {
        ...
        $.ajax({
            ...
            success: function (response) {
                    ...
                    rows += `
                        <tr>
                            ...
                            <td class="text-center d-flex">
                                ...
                                <form action="/admin/delete-assigned-task/${assigntask.id}" method="POST" onsubmit="return confirm('Are you sure you want to delete Task?');">
                ...
                const newUrl = `${window.location.origin}/admin/assigntask/index${query.toString() ? `?${query.toString()}` : ''}`;
